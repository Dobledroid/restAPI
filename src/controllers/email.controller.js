...
const senderEmail = 'sportgymcenterinfo@gmail.com';
const senderPassword = 'faol swhy ktje sibg';
const recipientEmail = '20210680@uthh.edu.mx';
...
export const validarIdentidad = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      <p>Alguien ha intentado acceder a tu cuenta. Si no fuiste tú, por favor, sigue las instrucciones a continuación.</p>
      <p style="text-align: center; color: green; font-size: 20px;"><strong>Sport Gym Center</strong></p>
      <br>
      <p style="font-size: 20px;">El token de confirmación es: <strong>${randomCode}</strong></p>
      <br>
      <p>Si fuiste tú quien intentó acceder a tu cuenta, utiliza el siguiente token para completar el proceso de confirmación de identidad.</p>
      <p>Si no fuiste tú quien intentó acceder, por favor, comunícate con el administrador de Sport Gym Center para reportar este incidente.</p>
      <p>Gracias por utilizar el sistema integral Sport Gym Center. Estamos aquí para ayudarte.</p>
      <br>
...
export const sendQRByEmail = async (qrImage, userEmail, fechaInicio) => {

  // Decodificar la imagen base64
  // const decodedImage = Buffer.from(qrImage.split(',')[1], 'base64');

  // // Guardar la imagen en el sistema de archivos local
  // fs.writeFile('qr_code_received.png', decodedImage, function(err) {
  //   if (err) {
  //     console.error('Error al guardar la imagen:', err);
  //   } else {
  //     console.log('Imagen guardada correctamente como qr_code_received.png');
  //   }
  // });
  // console.log("qrImage", qrImage);
  const nombreArchivo = `qr_code_${fechaInicio}.png`;
  const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
      user: senderEmail,
  ...
  const mailOptions = {
    from: senderEmail,
    to: userEmail,
    subject: '¡Bienvenido a Sport Gym Center!',
    html: `
    <h2>¡Bienvenido a Sport Gym Center!</h2>
    <p>Gracias por adquirir nuestra membresía en Sport Gym Center. Estamos emocionados de tenerte como parte de nuestra comunidad de fitness.</p>
    <p>Para acceder a nuestro gimnasio, simplemente presenta el código QR que se presenta al final del correo electrónico en la entrada.</p>
    <p>Una vez que hayas mostrado este código QR al personal de recepción, podrás disfrutar de todos nuestros servicios y actividades.</p>
    <p>Si tienes alguna pregunta o necesitas asistencia adicional, no dudes en contactarnos. Estamos aquí para ayudarte en tu viaje hacia una vida más saludable y activa.</p>
